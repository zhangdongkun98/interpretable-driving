...
class PartialDatasetAugment(PartialDataset):
    ...
    def __init__(self, config, path, process, pc_type='png'):
        ...
        '''pm and ipm'''
        self.pm = pm.PerspectiveMapping(config, process.K, process.T_img_imu)
        self.ipm = ipm.InversePerspectiveMapping(config, process.K, process.T_img_imu, z=self.imu_height, reverse_y=False)
        self.nm = nm.NavMapUtils(config)
    ...
    def generate_trajectory_types(self):
        ...
        #         continue
            ...
            if abs(avg) <= self.config.pm.type_threshold:
                type_id = 0
            elif avg > self.config.pm.type_threshold:
                type_id = 1
            else:
                type_id = 2
    ...
    def generate_costmap(self, ref_timestamp, pose_array):
        points = basic_utils.get_trajectory_points(pose_array, self.config.vehicle_width, self.config.correction_length, resolution=0.1)
        ...
        return get_costmap(self.config.ipm, img_ipm, pc)
    ...
    def save_image_pm(self, ref_timestamp, pose_array, T_w_imu):
        ...
        image_pm = image_pm.resize((self.config.image.image_width, self.config.image.image_height))
    ...
    def save_stereo_image(self, ref_timestamp):
        ...
        image = image.resize((self.config.image.image_width, self.config.image.image_height))
